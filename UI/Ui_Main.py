...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        #QtCore.QObject.connect(self.tableWidget.horizontalHeaderItem(0), QtCore.SIGNAL(_fromUtf8("itemClicked()")), self.test)   
        ...
        QtCore.QObject.connect(self.pushButton_all,  QtCore.SIGNAL(_fromUtf8("clicked()")),  self.all)  #　全选按钮
        self.first = 0
    ...
    def myconfig(self):
        ...
        # 重新连接sql用于更新
        QtGui.QMessageBox.warning(MainWindow,  u"成功提示",  u"更新成功，如果更改了仓库地址，请重新连接")
    ...
    def download(self):
        ...
        try:
            maxRow = len(self.Dir[0]) + len(self.Dir[1])
            localpath = str(QtGui.QFileDialog.getExistingDirectory(MainWindow, u"选择路径", '/',  QtGui.QFileDialog.ShowDirsOnly).toUtf8())
            print localpath
            ...
            QtGui.QMessageBox.warning(MainWindow,  u"下载完成",  u"下载完成")
        else:
            QtGui.QMessageBox.warning(MainWindow,  u"下载完成",  u"下载完成")
    ...
    def E_trash(self):
        """进入回收站事件"""
        MyRepo.cd('/root/' + str(Ui_Login.ui1.ID) + '/.trash')
        self.printlist()
    ...
    def enter(self,  row):
        ...
        if MyRepo.pwd() == '/root':   # 若是根目录
            Mysql = Ui_Login.ui1.Login
            command = "select ID from User where Username = "  + "'" + str(item.text().toUtf8()) + "'"
            Mysql.query(command)
            ID =  Mysql.fetch_queryresult()[0][0] # 这个为本地的仓库目录
            
        elif MyRepo.pwd().count('/') == 2 and MyRepo.pwd().count(str(Ui_Login.ui1.ID)) == 0:  # 如果是别人的第一层目录，那么就要根据不同的权限来看它能否进入了
            # 只有学生是不能进入的
            
            Mysql = Ui_Login.ui1.Login
            command = "select SID from Stu where ID = " +str(Ui_Login.ui1.ID)
            Mysql.query(command)
            ID = Mysql.fetch_queryresult(100)
            
            for id in ID:
                if MyRepo.pwd().split('/')[1] == str(id[0]):
                    QtGui.QMessageBox.warning(MainWindow,  u"发生错误",  u"你仅有访问顶层目录的权限")
                    return
        else:
    ...
    def printlist(self):
        ...
        for doc in self.Dir[0]:
            ...
            # 给文件添加大小
            size = MyRepo.size(doc)
            item = QtGui.QTableWidgetItem()
            item.setText(_translate("MainWindow",  str(size) + 'byte',  None))
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.lineEdit.setText(_translate("MainWindow", "/我的仓库/", None))
        self.pushButton_all.setText(_translate("MainWindow",  "全选",  None))
