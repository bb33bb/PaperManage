...
class Ui_Dialog(object):
    ...
    def Login(self):
        user = str(self.lineEdit.text().toUtf8())
        passwd = str(self.lineEdit_2.text())
        import ConnectSQL
        self.Login = ConnectSQL.ConnectSQL(admin.sql['host'], admin.sql['user'], admin.sql['passwd'], admin.sql['db'])
        import Check
        ...
        try:
            self.Login.query(command)
        except:
            QtGui.QMessageBox.warning(Dialog1,  u"发生错误",  u"无法连接网络，请检查网络是否已经连接")
            return
        result = self.Login.fetch_queryresult()
        ...
        try:
            if (result[0][1] == user or result[0][3] == user) and (result[0][2] == passwd):
                self.ID = result[0][0]
                Dialog1.close()
                # 查看是否已经有仓库，如果没有就是第一次登录的啦
                command = "select ID from Repo where ID = " + str(self.ID)
                self.Login.query(command)
                repo = self.Login.fetch_queryresult()
                if len(repo) == 0:
                    QtGui.QMessageBox.warning(Dialog1,  u"提醒",  u"由于你是第一次登录，请先进入设置页面选择您的的仓库的地址")
                import Ui_Main
        except:
