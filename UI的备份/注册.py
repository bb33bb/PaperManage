...
class Ui_Dialog(object):
    ...
    def Signin(self):
        ...
        #print info1,  info2,  info3,  info4,  info5,  info6,  info7,  info8,  info9,  info10,  info11
        
        
        # 还是一个一个挨着检查吧，检查的时候顺便把用户名info1和电子邮件info10的唯一性给检查了
        import Check
        import ConnectSQL
        Zhuce = ConnectSQL.ConnectSQL(user = 'yourgod', passwd = '896499825')
        ...
        if (not Check.isZhiwu(unicode(info7,  "utf-8"))) and (not info7.isEmpty()):
            QtGui.QMessageBox.warning(Dialog2,  u"发生错误",  u"请输入正确的职务信息（不得有特殊字符，长度为6-20）")
            return
